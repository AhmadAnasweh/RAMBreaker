...
class List_Files(plugins.PluginInterface):
    ...
    def _walk_vnode(cls, context, vnode, loop_vnodes):
        """
        Adds the given vnode, every vnode reachable through its v_mntvnodes
        tail-queue, and every vnode reachable through the parent chains.

        Iterative rewrite of what was originally a recursive walk. The old
        version recursed once per parent level (see git history: a call to
        _walk_vnode on each parent), which used one Python stack frame per
        directory-tree level and overflowed the interpreter recursion limit
        ("RecursionError: maximum recursion depth exceeded") on images whose
        vnode graph is deep or large. An explicit work stack has no such
        limit. Deduplication is still handled by _add_vnode keying on the
        vnode offset, so the set of vnodes collected is unchanged.
        """
        added = False
        pending = [vnode]
        ...
        while pending:
            ...
            # walk the v_mntvnodes tail-queue starting at this vnode
            while vnode:
                # _add_vnode returns False if the vnode is already recorded
                # (offset-keyed dedup) or is invalid/unnamed -> stop this chain
                if not cls._add_vnode(context, vnode, loop_vnodes):
                    break

                added = True

                # queue the parent for processing instead of recursing into it
                parent = cls._get_parent(context, vnode)
