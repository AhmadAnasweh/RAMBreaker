"""cmd_analyzer.linux.py — Linux-specific command analyzer (v6.0)

Contains Linux CMD_PATTERNS, SUSPICIOUS_CHAINS, MITRE_TECHNIQUES,
CommandAnalyzer, and MitreMapper for Linux memory images.
"""
...
class CommandAnalyzer:
    ...
    def analyze(self, output_dir: Path) -> Dict[str, Any]:
        ...
        self.log.info("Command analysis (linux): %d flags, %d chains, %d MITRE techniques",
                      len(flags), len(chains), len(mitre_hits))
        return {"flags": flags, "chains": chains, "mitre": mitre_hits, "os_type": "linux"}
    ...
    def write_report(self, output_dir: Path, results: Dict[str, Any]) -> Path:
        ...
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write("=" * 80 + "\n")
            f.write("  COMMAND LINE ANALYSIS — CresCent RAM Forensics Toolkit v6.0 (Linux)\n")
...
class MitreMapper:
    ...
    def write_report(self, output_dir: Path, results: Dict[str, Any]) -> Path:
        ...
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write("=" * 80 + "\n")
            f.write("  MITRE ATT&CK MAPPING — CresCent RAM Forensics Toolkit v6.0 (Linux)\n")
