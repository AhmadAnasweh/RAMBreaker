"""CresCent RAM Forensics Toolkit v6.0 - Scheduled Tasks Scanner (macOS)

Extracts launchd / LaunchAgent / LaunchDaemon evidence from macOS memory images.
Primary sources: Bash history (launchctl), lsof (plist files), list_files, processes (launchd/cfprefsd), envars.
...
class ScheduledTasksScanner:
    ...
    def scan(self, output_dir: Path) -> Dict[str, Any]:
        ...
        # macOS: launchd evidence, filescan (plist paths), process names, cmdlines
        self._scan_mac(jd)
        ...
        total = (len(self._file_tasks) + len(self._task_processes) +
                 len(self._suspicious_cmds) + len(self._mac_launchd))
