"""extractor.mac.py — macOS-specific Volatility plugin runner (v6.0)

Extracted from extractor.py for OS-specific split architecture.
Contains macOS PLUGINS only. No VOL2_EXCLUSIVE, no Windows-specific methods.
"""
...
PLUGINS: Dict[str, str] = {
    "vol3-mac-fast": "mac.pslist.PsList mac.pstree.PsTree mac.psaux.Psaux mac.bash.Bash mac.netstat.Netstat mac.lsmod.Lsmod mac.malfind.Malfind mac.list_files.List_Files mac.proc_maps.Maps",
    "vol3-mac-full": "mac.pslist.PsList mac.pstree.PsTree mac.psaux.Psaux mac.bash.Bash mac.netstat.Netstat mac.ifconfig.Ifconfig mac.lsmod.Lsmod mac.malfind.Malfind mac.check_syscall.Check_syscall mac.check_sysctl.Check_sysctl mac.check_trap_table.Check_trap_table mac.kauth_listeners.Kauth_listeners mac.kauth_scopes.Kauth_scopes mac.kevents.Kevents mac.list_files.List_Files mac.mount.Mount mac.proc_maps.Maps mac.socket_filters.Socket_filters mac.timers.Timers mac.trustedbsd.Trustedbsd",
    "vol3-mac-malware": "mac.pslist.PsList mac.pstree.PsTree mac.malfind.Malfind mac.check_syscall.Check_syscall mac.check_sysctl.Check_sysctl mac.check_trap_table.Check_trap_table mac.kauth_listeners.Kauth_listeners mac.socket_filters.Socket_filters mac.trustedbsd.Trustedbsd mac.lsmod.Lsmod",
    "vol3-mac-network": "mac.pslist.PsList mac.netstat.Netstat mac.ifconfig.Ifconfig mac.socket_filters.Socket_filters",
}
...
def _short_name(plugin: str) -> str:
    parts = plugin.lower().replace("mac.", "").split(".")
    if len(parts) >= 2:
        return parts[-2]
    return parts[0]
...
class Extractor:
    """Parallel Volatility plugin runner for macOS images."""
    ...
    def run(self, image: str, output_dir: Path, mode: str = "full",
        ...
        key = f"{self.vol.vol_version}-mac-{mode}"
        plugin_str = PLUGINS.get(key, PLUGINS.get("vol3-mac-full", ""))
        plugins = plugin_str.split()

        self.log.info("Starting macOS extraction: %d plugins (%s - %s mode, %d parallel jobs)",
