"""CresCent RAM Forensics Toolkit v6.0 - Scheduled Tasks Scanner (Windows)

Extracts scheduled task evidence from Windows memory images.
Primary sources: Registry (TaskCache), filescan (.job files), processes, cmdlines.
...
class ScheduledTasksScanner:
    ...
    def scan(self, output_dir: Path) -> Dict[str, Any]:
        ...
        # Windows: registry, filescan (.job files), process names, cmdlines
        self._scan_registry(jd)
        self._scan_filescan(jd)
        self._scan_processes(jd)
        self._scan_cmdlines(jd)

        total = (len(self._registry_tasks) + len(self._file_tasks) +
